    ...
    try {
        ...
        if (interaction.isCommand()) {
            ...
            if (action === 'accept') {
                ...
                playRequest.status = playRequest.acceptCount >= 2 ? 'Accepted' : playRequest.status;
                
                await updateRequesterPreview(playRequestId, playRequest.status);
                await interaction.update({
                    content: `Play Request ID: ${playRequestId}\n\nGame: ${playRequest.game}\nAccepts: ${playRequest.acceptCount}\nDenies: ${playRequest.denyCount}`,
                ...
                playRequest.status = playRequest.denyCount >= 2 ? 'Denied' : playRequest.status;
            ...
            const [action, playRequestId] = customId.split('_');
            const playRequest = playRequests.get(playRequestId);
