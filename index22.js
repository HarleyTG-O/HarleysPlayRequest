    ...
    return new ActionRowBuilder()
        .addComponents(
            ...
            new ButtonBuilder()
                ...
                .setStyle(ButtonStyle.Danger),
            new ButtonBuilder()
                .setCustomId(`warn_${playRequestId}`)
    ...
    try {
        ...
        if (interaction.isCommand()) {
            ...
            if (commandName === 'play') {
                ...
                // Send the preview embed to the notification channel
                const notificationChannel = await client.channels.fetch(NOTIFICATION_CHANNEL_ID);
                const notificationMessage = await notificationChannel.send({ embeds: [previewEmbed], components: [actionRow] });
