...
console.log('Request Menu Channel ID:', REQUEST_MENU_CHANNEL_ID);
        ...
        if (interaction.isCommand()) {
            ...
            if (commandName === 'play') {
                ...
                if (playRequests.has(id)) {
                    ...
                    await interaction.reply({ content: `Report for play request ${id} has been submitted.` });
                } else {
                    await interaction.reply({ content: 'Play request not found!', ephemeral: true });
                }
            }
            ...
            if (customId.startsWith('accept_')) {
                // Handle Accept button interaction
                const playRequestId = customId.split('_')[1];
                // Logic to accept the play request
                await interaction.reply({ content: `Play request ${playRequestId} accepted.` });
            } else if (customId.startsWith('deny_')) {
                // Handle Deny button interaction
                const playRequestId = customId.split('_')[1];
                // Logic to deny the play request
                await interaction.reply({ content: `Play request ${playRequestId} denied.` });
            } else if (customId.startsWith('report_')) {
                // Handle Report button interaction
                const playRequestId = customId.split('_')[1];
                // Logic to report the play request
                await interaction.reply({ content: `Play request ${playRequestId} reported.` });
            } else if (customId.startsWith('delete_')) {
                // Handle Delete button interaction
                const playRequestId = customId.split('_')[1];
                // Logic to delete the play request
                await interaction.reply({ content: `Play request ${playRequestId} deleted.` });
            } else if (customId.startsWith('ban_')) {
                // Handle Ban button interaction
                const userId = customId.split('_')[1];
                // Logic to ban the user
                await interaction.reply({ content: `User <@${userId}> has been banned.` });
            } else if (customId.startsWith('warn_')) {
                // Handle Warn button interaction
                const userId = customId.split('_')[1];
                // Logic to warn the user
                await interaction.reply({ content: `User <@${userId}> has been warned.` });
            }
        }
    } catch (error) {
