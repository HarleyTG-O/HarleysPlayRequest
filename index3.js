        ...
        if (interaction.isCommand()) {
            ...
            if (commandName === 'play') {
                ...
                if (playRequest.userId !== interaction.user.id) {
                    return interaction.reply({ content: 'You can only end your own play requests!', ephemeral: true });
                }
                ...
                const embed = new EmbedBuilder()
                    .setColor('#ff0000')
                    .setTitle('Play Request Ended')
                    .setDescription(`Play Request ID: ${playRequestId}\n\nStatus: Ended`);

                const notificationMessage = await client.channels.fetch(playRequest.notificationMessageId);
                await notificationMessage.edit({ embeds: [embed], components: [] });
