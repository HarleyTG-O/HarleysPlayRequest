...
const readPlayRequests = () => {
    try {
        ...
        if (typeof parsedData !== 'object' || parsedData === null) {
            throw new Error('Invalid data format in playRequests.json');
        }

        const map = new Map();
        Object.entries(parsedData).forEach(([key, value]) => {
            // Ensure acceptedBy and deniedBy are arrays before converting to Set
            if (!Array.isArray(value.acceptedBy)) {
                console.warn(`Invalid acceptedBy format for ${key}. Expected array, got ${typeof value.acceptedBy}`);
                value.acceptedBy = [];
            }

            if (!Array.isArray(value.deniedBy)) {
                console.warn(`Invalid deniedBy format for ${key}. Expected array, got ${typeof value.deniedBy}`);
                value.deniedBy = [];
            }

            value.acceptedBy = new Set(value.acceptedBy);
            value.deniedBy = new Set(value.deniedBy);
...
const writePlayRequests = (playRequests) => {
    try {
        const data = {};
        playRequests.forEach((value, key) => {
            data[key] = {
                ...value,
                acceptedBy: Array.from(value.acceptedBy),
                deniedBy: Array.from(value.deniedBy)
            };
        });
...
const notifyPlayRequest = async (playRequestId, game) => {
    try {
        const notificationChannel = await client.channels.fetch(NOTIFICATION_CHANNEL_ID);
        if (notificationChannel) {
            const notificationEmbed = new EmbedBuilder()
                .setColor('#0099ff')
                .setTitle('Play Request Details')
                .setDescription(`A new play request has been made!`)
                .addFields(
    ...
    try {
        ...
        if (interaction.isCommand()) {
            ...
            if (commandName === 'play') {
                ...
                // Notify on creation
                await notifyPlayRequest(playRequestId, game);
                ...
                if (user) {
                    ...
                    if (!bannedUsers.includes(user.id)) {
                        ...
                        await interaction.reply({ content: `${user.tag} has been banned from making play requests.`, ephemeral: true });
                    } else {
            ...
            if (playRequest) {
                if (action === 'accept') {
                    ...
                    // Send notification
                    await sendNotification('Play Request Accepted', `Play request ID ${playRequestId} has been accepted by <@${interaction.user.id}>.`);
                } else if (action === 'deny') {
                    ...
                    // Send notification
                    await sendNotification('Play Request Denied', `Play request ID ${playRequestId} has been denied by <@${interaction.user.id}>.`);
